...
const MyEventDetails = () => {
  ...
  // State to manage the active category and whether details are visible
  ...
  const formattedEvents = eventData.map(event => ({
    ...
    const [eventDetails] = useState(formattedEvents);

  // Get the current event based on the URL parameter
  const currentEvent = eventDetails.find(details => details.id.toString() === id )

  // If no event is found for the given id, display an error message.
  if (!currentEvent) {
  ...
  const renderCategoryContent = () => {
    const category = currentEvent.categories[activeCategory];
  ...
  return (
    <div>
        <div className="w-full px-2 sm:px-4 md:max-w-2xl md:mx-auto mt-28 mb-8 space-y-6 sm:space-y-8">
            <div className="rounded-lg md:rounded-[30px] overflow-hidden shadow-2xl">
                {/* Event Header */}
                <div
                ...
                <div className="relative z-10">
                    ...
                    <img
                        src={currentEvent.organizer.src}
                    ...
                    <p className="text-sm sm:text-base text-white/90 font-semibold leading-relaxed max-w-xs">
                        Hello, this is to welcome you all to the {currentEvent.title}.
                    </p>
                ...
                <div className="p-4 sm:p-6 bg-[#272222] transition-all duration-500 ease-in-out">
                    ...
                    <button
                        onClick={(e) => {
                        e.stopPropagation();
                        const categories = Object.keys(currentEvent.categories);
                    ...
                    <div className="flex items-center gap-2">
                        {currentEvent.categories[activeCategory].icon}
                        <span className="text-sm sm:text-base text-white font-medium">
                        {currentEvent.categories[activeCategory].title}
                        </span>
                    </div>
